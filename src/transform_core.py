...
class TransformCore:
    def __init__(self):
        self.databricks = DatabricksHandler()
    ...
    def process_dataframe(
        ...
        try:
            ...
            if search_description:
                ...
                matches = self.databricks.find_matching_rows(search_texts, search_description)
            ...
            for col, settings in column_commands.items():
                ...
                for i in range(0, len(matching_rows), BATCH_SIZE):
                    ...
                    for idx in batch_rows:
                        result_df.at[idx, output_col] = self.databricks.transform_text(
                            str(df.at[idx, col]),
                            settings['command']
                        )
                        completed += 1
    ...
    def generate_column(
        ...
        try:
            ...
            for i in range(0, len(df), BATCH_SIZE):
                ...
                batch_results = self.databricks.batch_generate_column(
                    batch_data, 
                    generation_prompt
                )
